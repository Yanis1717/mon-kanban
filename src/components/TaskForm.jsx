...
export default function TaskForm({ boardId, onCreated }) {
  ...
  const [assignedTo, setAssignedTo] = useState(''); // Nouveau state pour l'assignation
  const [categories, setCategories] = useState([]);
  const [profiles, setProfiles] = useState([]); // Nouveau state pour les utilisateurs
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState('');

  // Charger les catégories et les profils au montage
  useEffect(() => {
  ...
  async function handleSubmit(e) {
    ...
    const { error } = await supabase.from('tasks').insert([{
      ...
      assigned_to: assignedTo || null, // Envoi de l'utilisateur assigné en BDD
      created_by: user.id,
    }]);
    ...
    setCategoryId(''); 
    setDueDate('');
    setAssignedTo(''); // Réinitialisation du nouveau champ
  ...
  return (
    <form onSubmit={handleSubmit} style={{ background: 'white', padding: '1.5rem', borderRadius: '10px', marginBottom: '1.5rem', border: '1px solid #E2E8F0' }}>
      ...
      {/* Passage à 5 colonnes (1fr 1fr 1fr 1fr 1fr) pour intégrer le nouveau select */}
